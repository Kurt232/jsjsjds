  ...
  <div>
    ...
    <el-dialog
      ...
      <el-form :model="dataForm" :rules="dataRule" ref="dataFormRef" label-width="80px">
        ...
        <el-form-item label="事件中文名" prop="eventchinesename">
          <el-input v-model="dataForm.eventchinesename" placeholder=""></el-input>
        </el-form-item>
        <el-form-item label="事件英文名" prop="eventenglishname">
...
function getDataList() {
  ...
  if (input.key === '') {
    ...
    $http({
      url: `/data/data4/info/${input.key}`,
      method: 'get',
      params: {},
    }).then(({data}) => {
      if (data && data.code === 0 && data.data4 !== null) {
        dataList.value = [data.data4]
      } else {
        dataList.value = []
      }
      dataListLoading.value = false
...
function init(id) {
  ...
  if (dataForm.id) {
    $http({
      url: `/data/data4/info/${dataForm.id}`,
      method: 'get',
      params: {},
    }).then(({data}) => {
      if (data && data.code === 0) {
        dataForm.eventid = data.data4.eventid
        dataForm.caseid = data.data4.caseid
        dataForm.eventchinesename = data.data4.eventchinesename
        dataForm.eventenglishname = data.data4.eventenglishname
      }
    })
  }
}
...
function dataFormSubmit() {
  dataFormRef.value.validate((valid) => {
    if (valid) {
      $http({
        url: `/data/data4/${!dataForm.id ? 'save' : 'update'}`,
        method: 'post',
        data: {
