...
function getDataList() {
  ...
  if (input.key === '') {
    ...
    $http({
      url: `/user/drugmessage/info/${input.key}`,
      method: 'get',
      params: {},
    }).then(({data}) => {
...
function init(id) {
  ...
  if (dataForm.id) {
    $http({
      url: `/user/drugmessage/info/${dataForm.id}`,
      method: 'get',
      params: {},
    }).then(({data}) => {
      if (data && data.code === 0) {
        dataForm.atccategorychinese = data.drugmessage.atccategorychinese
        dataForm.atccategoryenglish = data.drugmessage.atccategoryenglish
        dataForm.drugchinesename = data.drugmessage.drugchinesename
        dataForm.drugenglishname = data.drugmessage.drugenglishname
      }
    })
  }
}
...
function dataFormSubmit() {
  dataFormRef.value.validate((valid) => {
    if (valid) {
      $http({
        url: `/user/drugmessage/${!dataForm.id ? 'save' : 'update'}`,
        method: 'post',
        data: {
