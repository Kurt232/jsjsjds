...
function getDataList() {
  ...
  if (input.key === '') {
    ...
    $http({
      url: `/user/drug/info/${input.key}`,
      method: 'get',
      params: {},
    }).then(({data}) => {
      if (data && data.code === 0 && data.drug !== null) {
        dataList.value = [data.drug]
      } else {
        dataList.value = []
      }
      dataListLoading.value = false
...
function init(id) {
  ...
  if (dataForm.id) {
    $http({
      url: `/user/drug/info/${dataForm.id}`,
      method: 'get',
      params: {},
    }).then(({data}) => {
      if (data && data.code === 0) {
        dataForm.uid = data.drug.uid
        dataForm.drugid = data.drug.drugid
      }
    })
  }
}
...
function dataFormSubmit() {
  dataFormRef.value.validate((valid) => {
    if (valid) {
      $http({
        url: `/user/drug/${!dataForm.id ? 'save' : 'update'}`,
        method: 'post',
        data: {
